...
module.exports = (() => {
  var _ref = _asyncToGenerator(function* (cache, options) {
    ...
    while (requiredModulePaths.length > 0) {
      ...
      if (!moduleASTs[relativeFilePath]) {
        ...
        const transform = new FileRequireTransform({
          ...
          didFindRequire: function (unresolvedPath, resolvedPath) {
        ...
        const cachedTransform = yield cache.get({ filePath, content: source });
        const useCachedTransform = cachedTransform ? cachedTransform.requires.every(function (r) {
          return (transform.resolveModulePath(r.unresolvedPath) || r.unresolvedPath) === r.resolvedPath;
        }) : false;
        ...
        if (useCachedTransform) {
          ...
          yield cache.put({ filePath, original: source, transformed: transformedSource, requires: foundRequires });
        }

        moduleASTs[relativeFilePath] = `function (exports, module, __filename, __dirname, require, define) {\n${transformedSource}\n}`;
        requiredModulePaths.push(...foundRequires.map(function (r) {
          return r.resolvedPath;
        }));
      }
    }

    yield cache.deleteUnusedEntries();
    ...
    snapshotScript = snapshotScript.slice(0, auxiliaryDataAssignmentStartIndex) + `var snapshotAuxiliaryData = ${auxiliaryData};` + snapshotScript.slice(auxiliaryDataAssignmentEndIndex);
    ...
    snapshotScript = snapshotScript.slice(0, definitionsAssignmentStartIndex) + `customRequire.definitions = {\n${definitions}\n  };` + snapshotScript.slice(definitionsAssignmentEndIndex);
    ...
    snapshotScript = snapshotScript.slice(0, sectionsAssignmentStartIndex) + `snapshotAuxiliaryData.snapshotSections = ${JSON.stringify(sections)}` + snapshotScript.slice(sectionsAssignmentEndIndex);
