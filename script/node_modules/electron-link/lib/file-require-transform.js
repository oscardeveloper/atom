...
const NODE_CORE_MODULES = new Set(['assert', 'buffer', 'child_process', 'cluster', 'constants', 'crypto', 'dgram', 'dns', 'domain', 'events', 'fs', 'http', 'https', 'module', 'net', 'os', 'path', 'punycode', 'querystring', 'readline', 'repl', 'stream', 'string_decoder', 'tls', 'tty', 'url', 'util', 'v8', 'vm', 'zlib']);
  ...
  replaceAssignmentOrDeclarationWithLazyFunction(astPath) {
    ...
    while (parentPath != null && parentPath.scope === astPath.scope) {
      ...
      if (parentNode.type === 'AssignmentExpression') {
        ...
        assert(astPath.scope.declares(assignmentLhs.name), `${this.options.filePath}\nAssigning a deferred module to a variable that was not declared in this scope is not supported!`);

        // Replace assignment with lazy function
        let replacement = null;
        if (ifStatementPath) {
          if (!this.lazyRequireFunctionsByVariableName.has(parentNode.left.name)) {
            ifStatementPath.insertBefore(b.variableDeclaration('let', [b.variableDeclarator(b.identifier(lazyRequireFunctionName), null)]));
          }

          replacement = b.assignmentExpression('=', b.identifier(lazyRequireFunctionName), b.functionExpression(null, [], b.blockStatement([b.returnStatement(b.assignmentExpression('=', parentNode.left, b.logicalExpression('||', parentNode.left, parentNode.right)))])));
        } else {
          replacement = b.functionDeclaration(b.identifier(lazyRequireFunctionName), [], b.blockStatement([b.returnStatement(b.assignmentExpression('=', parentNode.left, b.logicalExpression('||', parentNode.left, parentNode.right)))]));
        }
        ...
        if (parentNode.id.type === 'ObjectPattern') {
          ...
          for (const property of parentNode.id.properties.slice().reverse()) {
            ...
            variableDeclarationPath.insertAfter(b.functionDeclaration(b.identifier(lazyRequireFunctionName), [], b.blockStatement([b.returnStatement(b.assignmentExpression('=', property.key, b.logicalExpression('||', property.key, b.memberExpression(parentNode.init, property.key))))])));
          ...
          variableDeclarationPath.insertAfter(b.functionDeclaration(b.identifier(lazyRequireFunctionName), [], b.blockStatement([b.returnStatement(b.assignmentExpression('=', parentNode.id, b.logicalExpression('||', parentNode.id, parentNode.init)))])));
...
function isStaticRequire(astPath) {
  ...
  return node.callee.name === 'require' && node.arguments.length === 1 && node.arguments[0].type === 'Literal';
}

function isStaticRequireResolve(astPath) {
  const node = astPath.node;
  return node.callee.type === 'MemberExpression' && node.callee.object.name === 'require' && node.callee.property.name === 'resolve' && node.arguments.length === 1 && node.arguments[0].type === 'Literal';
}
